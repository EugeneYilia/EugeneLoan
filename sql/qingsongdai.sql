...
CREATE TABLE `borrow_money_flow` (
  ...
  `in_bound_account` varchar(30) NOT NULL COMMENT '钱转入账户',
  `out_bound_account` varchar(30) NOT NULL COMMENT '钱转出账户',
...
CREATE TABLE `intend_borrow` (
  `bill_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '账目id',
  `id_card` char(18) NOT NULL,
  `intend_money` double(15,3) NOT NULL,
  `start_date` datetime NOT NULL,
  `pay_rate` float(5,5) NOT NULL,
  `pay_type` int(1) NOT NULL COMMENT '还款方式按月付 季付,算利息的方式，不影响limit_months的填写',
  `limit_months` int(2) NOT NULL COMMENT '一个月 三个月 半年',
...
CREATE TABLE `intend_lend` (
  `bill_id` int(11) NOT NULL COMMENT '和意向借入表中的账目id是一致的',
  `id_card` char(18) NOT NULL,
...
CREATE TABLE `recharge_record` (
  `serial_number` int(11) NOT NULL AUTO_INCREMENT COMMENT '充值资金流水id',
...
CREATE TABLE `trade` (
  ...
  `should_repay_principal` double(15,3) NOT NULL COMMENT '每次已还本金',
  `should_repay_interest` double(15,3) NOT NULL COMMENT '每次应还利息',
  `should_repay_liquidated_money` double(15,3) NOT NULL COMMENT '总共应还违约金',
  `finished_date` datetime NOT NULL COMMENT '最终贷款还清日期',
  PRIMARY KEY (`serial_number`),
  KEY `bill_id` (`bill_id`),
  KEY `in_bound_account` (`in_bound_account`),
  KEY `out_bound_account` (`out_bound_account`),
...
CREATE TABLE `user_optional_info` (
  ...
  `educational_level` varchar(30) DEFAULT NULL,
  `marriage` varchar(30) DEFAULT NULL,
  `profession` varchar(30) DEFAULT NULL,
  `address` varchar(100) DEFAULT NULL,
  `avatar` mediumblob DEFAULT NULL,
  `special_identity` varchar(30) DEFAULT NULL,
  PRIMARY KEY (`id_card`),
...
CREATE TABLE `withdraw_record` (
  `serial_number` int(11) NOT NULL AUTO_INCREMENT COMMENT '提现资金流水id',
